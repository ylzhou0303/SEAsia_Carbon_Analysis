...
condition = 'clear';
season = '';  % for Talang, enter spring, summer, fall or winter; for SG, just leave it empty
...
% CDOM AQY using PSLR and gradiant descent method (Zhu et al. 2020)
load('C:\NTU\Research\Photodegradation experiment\manuscript\Frontiers in Marine Science\Review and Revisions\CDOM_AQY_Zhu.mat','Beta_opt');
...
%AQY_CDOM_spc = AQY_CDOM_250_700{:,2:end};  %use the CDOM AYQ calculated
%using the original method (assuming exponential decay)
%by assuming an exponential decay
AQY_CDOM_spc = Beta_opt(11:end, 21:end)';  %use the CDOM AQY calculated by Zhu et al. method (PLSR + gradient descent)
...
for d = 2:days  
    ...
    % DOC
    DOC_t.DOCloss(d-1) = sum(Qperday .* AQY_DOC_spc); %unit: mol
...
save(['C:\NTU\Research\Photodegradation experiment\photodeg modelling\model results\',savename]);
